...
export const generatePromptGuest = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: raw }) => {
    ...
    const userBlock = [
      ...
      `Main genre: ${data.mainGenre} · Era: ${data.era}`,
      data.fusionGenre !== "None" && `Fusion: ${data.fusionGenre}`,
      `Vocals: ${data.vocalType} — ${data.vocalPerformance} · Register: ${data.vocalRegister} · Texture: ${data.vocalTexture} · Harmony: ${data.harmonyStyle}`,
      ...
      tempoLine,
      `Key: ${data.key}`,
      `Production: ${data.productionStyle} · ${data.soundQuality} · Dynamics: ${data.dynamicsArc}`,
      `Stereo image: ${data.stereoCharacter}`,
      data.referenceTraits.length > 0 && `Reference traits: ${data.referenceTraits.join(", ")}`,
      `Engine optimization: ${data.optimizationMode}`,
      avoidCombined && `AVOID: ${avoidCombined}`,
    ].filter(Boolean).join("\n");
